	...
	do {
		...
		case 1: 
		{
			...
			do {
				...
				system("cls");
				cout << "[1] Получить отрицательные значения элементов(перегружен оператор !)" << endl << "[2] Уменьшить все элементы на 1(Префиксный оператор --)." << endl << "[3] Увеличить значение всех эелементов на 0.5(Оператор ++ (Префиксная форма))." << endl << "[4] Извлечь элемент из очереди( Оператор --(Постфиксная форма))." << endl << "[5] Добавить элемент в очередь(Оператор ++ (Постфиксная форма))." << endl <<  "[0] Прейти к другому заданию ил выйти." << endl;
				cin >> choice1;
